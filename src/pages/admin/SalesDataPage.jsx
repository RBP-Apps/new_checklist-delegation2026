...
function AccountDataPage() {
  ...
  // UPDATED: Admin Done submission handler
  ...
  const fetchSheetData = useCallback(async () => {
    try {
      ...
      rows.forEach((row, rowIndex) => {
        ...
        // ✅ ONLY ADD IF COLUMN K IS EMPTY/NULL (Actual column)
        const isColumnKEmpty = isEmpty(columnKValue);

        if (isColumnKEmpty && isEmpty(columnPValue)) {
          pendingAccounts.push(rowData);
        }
        // For history view, include tasks where Column K is NOT empty (has actual date)
        else if (!isColumnKEmpty) {
          // Admin sees all history, regular users see only their tasks
        ...
        <div className="rounded-lg border border-purple-200 shadow-md bg-white overflow-hidden">
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 border-b border-purple-100 p-4 flex justify-between items-center">
            ...
            <h1 className="sm:text-xl tracking-tight text-purple-700 text-center sm:text-left">
              {showHistory ? "History Tasks" : `Pending Tasks: ${filteredAccountData.length}`}
            </h1>
          </div>
  ...
  <label
    ...
    className="block text-xs font-medium text-purple-700 mb-1"
  >
    Filter by Member:
  </label>
  <select
