...
const APPS_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbzOtWdWnKR-Nu-N4RvLdOb7pT4ZtvXBlWI1XaaTgGMM0uG06zyKJOrIDu4rvS6IYG_z0w/exec";
...
function WorkingDate() {
  ...
  const getLastUniqueNumber = async () => {
    try {
      ...
      const response = await fetch(
        `${APPS_SCRIPT_URL}?action=fetch&sheetName=WorkingDate`,
      );
  ...
  const fetchHistory = async () => {
    ...
    try {
      const response = await fetch(
        `${APPS_SCRIPT_URL}?sheetName=WorkingDate&action=fetch`,
      );
      const data = await response.json();
  ...
  // UPDATED: Handle submit with proper quantity and message submission
  const handleSubmit = async () => {
    ...
    try {
      ...
      const rowsToSubmit = [];
      for (const timeSlot of filledSlots) {
        const details = workingDetails[timeSlot] || "";
        currentId++;
        const uniqueNumber = formatUniqueNumber(currentId);
        const qty = String(quantities[timeSlot] || "");

        // Detailed Column Mapping Log
        console.group(`Preparing Submission Row for ${timeSlot}`);
        console.log(`📍 Column A (Timestamp): ${timestamp}`);
        console.log(`📍 Column B (Date): ${formattedDate}`);
        console.log(`📍 Column C (Unique Number): ${uniqueNumber}`);
        console.log(`📍 Column D (Name): ${username}`);
        console.log(`📍 Column E (Time): ${timeSlot}`);
        console.log(`📍 Column F (Working Details): "${details}"`);
        console.log(`📍 Column G (Quantity): "${qty}"`);
        console.groupEnd();

        // FIXED: Ensure all values are strings
        rowsToSubmit.push([
          timestamp, // A: Timestamp
          formattedDate, // B: Date (DD/MM/YYYY)
      ...
      console.log("Batch submitting filled slots:", JSON.stringify(rowsToSubmit));

      // Step 3: Single submission call - FIXED URL structure
      const submitUrl = `${APPS_SCRIPT_URL}`;

      const formData = new URLSearchParams();
      formData.append('sheetName', 'WorkingDate');
      formData.append('action', 'insert');
      formData.append('batchInsert', 'true');
      formData.append('rowData', JSON.stringify(rowsToSubmit));

      const response = await fetch(submitUrl, {
        method: "POST",
        mode: "no-cors", // no-cors for Google Apps Script
        headers: {
      ...
      setSuccessMessage(
        `Successfully submitted ${rowsToSubmit.length} working detail(s) with quantity and message!`,
      );
      ...
      if (activeTab === "history") {
        setTimeout(() => {
          fetchHistory();
        }, 2000); // Wait 2 seconds for the data to be written
      }
                    ...
                    {historyData.map((item, idx) => (
                      <div key={idx} className="p-4 space-y-3 bg-white">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-800 font-medium leading-tight">{item.details || "No details provided"}</p>
                        </div>
                        ...
                        <div className="flex justify-between items-end pt-1">
                          ...
                          <div className="text-right">
                            ...
                            <span className="text-[10px] text-gray-500 font-medium italic">{formatHistoryTimestamp(item.timestamp)}</span>
                          </div>
                        </div>
                      </div>
                    ))}
